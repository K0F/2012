...
/**
 * See http://www.ecma-international.org/ations/standards/Ecma-048.htm
...
class Terminal{
  ...
  int[] screen;
  int[] screen2;
  State vt100_parse_state = State.None;
  int vt100_parse_len;
  int vt100_lastchar;
  int vt100_parse_func;
  String vt100_parse_param;
  boolean vt100_mode_autowrap;
  boolean vt100_mode_insert;
  boolean vt100_charset_is_single_shift;
  boolean vt100_charset_is_graphical;
  boolean vt100_mode_lfnewline;
  boolean vt100_mode_origin;
  boolean vt100_mode_inverse;
  boolean vt100_mode_cursorkey;
  boolean vt100_mode_cursor;
  boolean vt100_mode_alt_screen;
  boolean vt100_mode_backspace;
  boolean vt100_mode_column_switch;
  boolean vt100_keyfilter_escape;
  int[] vt100_charset_graph = new int[] {
  ...
  int vt100_charset_g_sel;
  int[] vt100_charset_g = { 
    0, 0
  };
  Map<String, Object> vt100_saved;
  Map<String, Object> vt100_saved2;
  int vt100_saved_cx;
  int vt100_saved_cy;
  String vt100_out;
  ...
  List<Integer> tab_stops;

  int utf8_char;
  int utf8_units_count;
  int utf8_units_received;

  AtomicBoolean dirty = new AtomicBoolean(true);

  Terminal(PApplet _parent) {
    
    this(_parent, 80, 24);
  }

  Terminal(PApplet _parent, int width, int height) {
    parent = _parent;
    this.width = width;
    this.height = height;
    reset_hard();
  ...
  int[] peek(int y0, int x0, int y1, int x1) {
  ...
  void poke(int y, int x, int[] s) {
    System.arraycopy(s, 0, screen, width * y + x, s.length);
    setDirty();
  }

  void fill(int y0, int x0, int y1, int x1, int c) {
    int d0 = width * y0 + x0;
  ...
  void clear(int y0, int x0, int y1, int x1) {
  ...
  void scroll_area_up(int y0, int y1) {
    scroll_area_up(y0, y1, 1);
  }

  void scroll_area_up(int y0, int y1, int n) {
    n = Math.min(y1 - y0, n);
    poke(y0, 0, peek(y0 + n, 0, y1, width));
    clear(y1-n, 0, y1, width);
  }

  void scroll_area_down(int y0, int y1) {
    scroll_area_down(y0, y1, 1);
  }

  void scroll_area_down(int y0, int y1, int n) {
    n = Math.min(y1 - y0, n);
    poke(y0 + n, 0, peek(y0, 0, y1-n, width));
    clear(y0, 0, y0 + n, width);
  }

  void scroll_area_set(int y0, int y1) {
  ...
  void scroll_line_right(int y, int x) {
    scroll_line_right(y, x, 1);
  }

  void scroll_line_right(int y, int x, int n) {
    if (x < width) {
  ...
  void scroll_line_left(int y, int x) {
    scroll_line_left(y, x, 1);
  }

  void scroll_line_left(int y, int x, int n) {
    if (x < width) {
      n = Math.min(width - cx, n);
  ...
  int[] cursor_line_width(int next_char) {
  ...
  void csi_ICH(String p) {
    int[] ps = vt100_parse_params(p, new int[] { 
      1
    }
    );
  ...
  void csi_CNL(String p) {
    csi_CUD(p);
    ctrl_CR();
  }

  void csi_CPL(String p) {
    csi_CUU(p);
    ctrl_CR();
  }

  void csi_CHA(String p) {
    int[] ps = vt100_parse_params(p, new int[] { 
      1
    }
    );
  ...
  void csi_ED(String p) {
    String[] ps = vt100_parse_params(p, new String[] { 
      "0"
    }
    );
    if ("0".equals(ps[0])) {
  ...
  void csi_HPR(String p) {
    csi_CUF(p);
  }

  void csi_REP(String p) {
    int[] ps = vt100_parse_params(p, new int[] { 
      1
    }
    );
  ...
  void csi_VPR(String p) {
    csi_CUD(p);
  }

  void csi_HVP(String p) {
    csi_CUP(p);
  }

  void csi_TBC(String p) {
    String[] ps = vt100_parse_params(p, new String[] { 
      "0"
    }
    );
  ...
  void csi_SM(String p) {
    vt100_setmode(p, true);
  }

  void csi_RM(String p) {
    vt100_setmode(p, false);
  }

  void csi_SGR(String p) {
    int[] ps = vt100_parse_params(p, new int[] { 
      0
    }
    );
  ...
  void csi_SCP(String p) {
    vt100_saved_cx = cx;
    vt100_saved_cy = cy;
  }

  void csi_RCP(String p) {
    cx = vt100_saved_cx;
    cy = vt100_saved_cy;
  }

  void csi_DECREQTPARM(String p) {
    String[] ps = vt100_parse_params(p, new String[0]);
    if ("0".equals(ps[0])) {
  ...
  String[] vt100_parse_params(String p, String[] defaults) {
    String prefix = "";
    if (p.length() > 0) {
  ...
  int[] vt100_parse_params(String p, int[] defaults) {
    String prefix = "";
    p = p == null ? "" : p;
    if (p.length() > 0) {
      if (p.charAt(0) >= '<' && p.charAt(0) <= '?') {
  ...
  void vt100_parse_reset() {
    vt100_parse_reset(State.None);
  }

  void vt100_parse_reset(State state) {
        ...
        if (c < 32) {
          ...
          if (vt100_parse_len > 32) {
            ...
            if (msb == 0x20) {
              ...
              vt100_parse_param += new String(new char[] { 
                (char) c
              } 
              );
            } 
  ...
  synchronized boolean setSize(int w, int h) {
  ...
  synchronized String pipe(String d) {
    String o = "";
    for (char c : d.toCharArray()) {
  ...
  synchronized boolean write(String d) {
    d = utf8_decode(d);
  ...
  synchronized String dump(long timeout, boolean forceDump) throws InterruptedException {
  ...
  String to_String() {
    ...
    for (int y = 0; y < this.height; y++) {
      for (int x = 0; x < this.width; x++) {
        sb.append((char) (this.screen[y * this.width + x] & 0xffff));
      }
      sb.append("\n");
    }
    return sb.toString();
  ...
  void to_Screen(int _x, int _y) {
    for (int y = 0; y < this.height; y++) {
      parent.text(" ",_x,y*12+_y);
      
      for (int x = 0; x < this.width; x++) {  
     // parent.fill(random(255),random(255),random(255));
     
         if((char)(this.screen[y * this.width + x] & 0xffff) == 'l')
         parent.fill(255,0,0);
         
        parent.text((char) (this.screen[y * this.width + x] & 0xffff));
      }
    }
  }
