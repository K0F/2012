void jedna(){
  ...
  for (float q = 0;q<width;q+=width/4) {
    ...
    {
      stroke(lerpColor(#473B0B, #ffffff, constrain(norm(vals[i], 0, 127), 0, 0.99)), vals[i]/8.0*map(ctl[46], 0, 127, 0, 8));
      vals[i] += (fft.getBand(i)*i*(reaktivita/32.0) - vals[i])/(127-ctl[34]+1.0);
