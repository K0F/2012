...
ArrayList p;
String filenames[] = {"souhvezdi.txt","people.txt","brain.txt","lines.txt","city.txt"};
int cycle = 0;
...
void draw() {
  ...
  text("fig. "+(cycle+1),width/4*3.2,height-50);
...
class Linka {
  ...
  void draw() {
    ...
    if(pulsating)
      stroke(255, (sin((frameCount+id*80.0)/100.0)+0.5)/2.0*255.0 );
