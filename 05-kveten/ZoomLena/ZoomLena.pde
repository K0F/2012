/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/7919*@* */
/* !do not delete the line above, required for linking your tweak if you re-upload */
	...
	void setup() {
		...
		lenna = loadImage("lenna256x256.gif");
	...
	private void applyWarp() {
		itsWarpGrid.update();
		lenna.loadPixels();
		itsWarpGrid.applyWarp(lenna.pixels);

    //tint(255,15);
    //image(itsWarpGrid.bufferImage,0,0);


		blend(itsWarpGrid.bufferImage,0,0,width,height,0,0,width,height,ADD);
		blend(itsWarpGrid.bufferImage,0,0,width,height,0,0,width,height,MULTIPLY);
	}
	...
	public class WarpGrid {
		...
		private float diffuseFactor = 0.21f;
		...
		private void update() {
			...
			// TODO: Grid-unabhängigen Datentyp für Warp erstellen
		...
		public int getColor(VectorType subPixelPosition, int[] sourcePixels) {
			...
			y = constrain(y, 0, bufferImage.height - 2);
			x = constrain(x, 0, bufferImage.width - 2);
