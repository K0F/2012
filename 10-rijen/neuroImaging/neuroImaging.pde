...
void draw() {
  ...
  translate((noise(millis()/100.0, 0)-0.5)*tras, (noise(0, millis()/100.0)-0.5)*tras);
...
pushMatrix();
translate((noise(millis()/33.0, 0)-0.5)*tras/2.0, (noise(0, millis()/33.0)-0.5)*tras/2.0);
  image(grid, 0, 0);
popMatrix();
  if (rec)
