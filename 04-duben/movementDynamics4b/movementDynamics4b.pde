...
class Rot {
  ...
  void draw() {
    ...
    w+=(-w)/3.0;
     ...
     PVector tr = new PVector(0,0);
     if(trail.size()>0)
    tr = (PVector)trail.get(0);
    
    x -= (tr.x-x)/((log((w*35.0)/(9000.1/w)+1.01)*(10.8+d2)));
    y -= (tr.y-y)/((log((w*35.0)/(9000.1/w)+1.01)*(10.8+d2)));
    ...
    x += (mouseX-x)/((log((w*35.0)/(9000.1/w)+1.01)*(10.0+d2)));
    y += (mouseY-y)/((log((w*35.0)/(9000.1/w)+1.01)*(10.0+d2)));
