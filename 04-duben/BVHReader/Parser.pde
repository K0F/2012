...
class Parser {
  ...
  void recurseTree(Node in) {
    ...
    for (int i = 0;i<ch.size();i++) {
      ...
      if (depth>0) {
        ...
        one.rot.y = 0;//noise(frameCount/(i+10.0))*10.0;//((sin(frameCount/41.0+depth/3.0))*15.);
        one.rot.z = 0;//(noise(frameCount/14.0+depth/30.0)*9.);
      }
  ...
  void setNodes(ArrayList _nodes){
    nodes = _nodes;
  }
  ...
  void draw() {
    ...
    for (int i = 2 ; i< nodes.size();i++) {
      ...
      Particle p = (Particle)dyn.particles.get(i);
      
      //node.pos.x = p.position().x();
      //node.pos.y = p.position().y();
      //node.pos.z = p.position().z();
